...
The pictures used for the first part comes from the [Labelled Face in the Wild Dataset](http://vis-www.cs.umass.edu/lfw/). The second dataset used is a small personnalized celebrity dataset. You can find all the data at this [link](https://drive.google.com/drive/folders/13ySIhIpwhYiprQblIZNIuUms9-mD3LoH?usp=sharing).
...
## Useful definitions of face recognition problems
**Face verification :** 
A face verification problem consist in checking if two input images are the same person or not.  

**Face recognition :**
A face recognition problem consist in checking if the input image corresponds to one of the person in a database. It can be done, for example, by running multiple face verifications between the input image and the entire dataset. This method can be assimilated to _one shot learning_ .  

**One shot learning problem :**
One-shot learning is an object categorization problem. Whereas most deep learning algorithms require training on thousands of samples and very large datasets, one-shot learning aims to learn information about object categories from one, or only few training samples.

**Open set problem :** 
An open-set classification is a problem of handling unknown classes that are not contained in the training dataset, whereas traditional classifiers assume that only known classes appear in the test environment.



## Main approach for face recognition problems :

Most of the face recognition use cases are open set problems. It can also be assimilated to one shot learning problems in the sense that there is often only few training examples to classify each identities. In this context, it can be difficult to specialize a neural network on each specific face recognition problems.

The main appraoch to solve those challenges consist, firstly in learning *face embeddings* one a closed set of identities on a large dataset and secondly using those vectorial/hidden representations to specific face verification or recognition task. For example it is possible to execute a face verification task between two faces just by examinate similarity of their two vectorial representations (even if those identities were not learned originally). 
...
### Loss presentation
We implemented the three following losses above a pre trained MobileNet architecture.

#### 1) The Triplet loss
Let A the hidden representation of a face, P the hidden representation of another face with the same identity than A and N the hidden representation of a face with another identity.
The triplet loss aim to minimize : 
![triplet](images/triplet.PNG)

Where alpha is a margin parameter (the higher it is, the more the loss is "permissive"). The idea of this loss function is to pass three examples (an Anchor a Postive and a Negative) through the neural network  instead of one to try to separate as much as possible the hidden representations of the different identities.
...
The sphere face loss is a slightly modified [A-softmax loss function](https://towardsdatascience.com/additive-margin-softmax-loss-am-softmax-912e11ce1c6b) described as the following : 
...
We did not understand all the geometric specifities of this loss, however it seems to push the network to separate as much as possible the embedding of the different classes.

### Results
We trained for 25 epochs two versions of each network/loss (changing the losses hyperparameters). Our embeding size is of 256 neurons. We tried to vizualize our representations with a TSNE algorithm. You can find all the losses and embedding graphs below. You can find more interpretations and plot in the **Part1 Notebook** but, to sum up, none of our networks achieved good results (in term of accuracy or embeddings).
...
Many things could have been enhanced to have better results.
#### Data
...
To do so, embeded each face using a pre trained Facenet model and try to  measure cosine similarity between the embeddings of the test set and the one of the identification set. If the similarity is below a certain threshold we can consider that the pictures have the same identity. If no picture came below the threshold we consider that the person is unknown.
...
Playing with the threshold hyperparameter and the number of pictures by identity in the identification set, we achieved really good results in term of accuracy and confusion matrix.
